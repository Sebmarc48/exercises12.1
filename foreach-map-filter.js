...
function valTimesIndex(arr) {
  //So what i have to do is pass the array value and the index as parameters to map and multiply the value by the index then save the returned array to an object and return it.
...
function extractKey(arr, key) {
  const valArr = arr.map(function (val) {
    return val[key];
  });
  return valArr;
}
...
function extractFullName(arr) {
  //get the values of the key "first" and "last" to get the full name;
...
Examples:
    filterByValue([{first: 'Elie', last:"Schoppik"}, {first: 'Tim', last:"Garcia", isCatOwner: true}, {first: 'Matt', last:"Lane"}, {first: 'Colt', last:"Steele", isCatOwner: true}],
     'isCatOwner') // [{first: 'Tim', last:"Garcia", isCatOwner: true}, {first: 'Colt', last:"Steele", isCatOwner: true}]
*/

function filterByValue(arr, key) {
  const arrToReturn = arr.filter(function (val) {
...
function find(arr, searchValue) {
  ...
  if (arrToReturn.includes(searchValue)) {
...
function removeVowels(str) {
  ...
  const vowels = "aeiou";
  const arrStr = caseInsensitiveStr.split("");
  let resultArr = arrStr.filter(function (val) {
...
function doubleOddNumbers(arr) {
  ...
  const doubledArr = arr.filter(function (num) {
    return num % 2 !== 0;
  });
  //then double them
  return doubledArr.map(function (num) {
    return num * 2;
  });
}
